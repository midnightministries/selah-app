...
const SELAH_CREAM = "var(--wm, #ece0c6)";   // cream on dark palettes; deep tone on light (each palette sets --wm)
...
// Small inline "?" that pops a short, plain explanation for a settings section.
// Hidden entirely when guidance is turned off. Resets its own text styling so it
// reads as prose even when it sits inside an all-caps label.
function HelpDot({ text, show = true }) {
  const [open, setOpen] = useState(false);
  // Always render so the layout never shifts when guidance is toggled; just hide it.
  return (
    <span style={{ position: "relative", display: "inline-flex", marginLeft: 8, verticalAlign: "middle", visibility: show ? "visible" : "hidden", pointerEvents: show ? "auto" : "none" }}>
      <button onClick={(e) => { e.stopPropagation(); setOpen(o => !o); }} aria-label="Help"
        style={{ width: 21, height: 21, boxSizing: "border-box", borderRadius: "50%", border: "1px solid rgba(142,28,28,0.4)", background: "transparent", color: CROSS_RED, textShadow: "0 0 6px rgba(142,28,28,0.55)", fontFamily: "-apple-system, system-ui, sans-serif", fontSize: 13, fontWeight: 400, lineHeight: 1, cursor: "pointer", padding: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>?</button>
      {show && open && (
...
function ExportSheet({ session, onClose }) {
  ...
  // Layer button: step selection to the next visible piece (so a buried element is
    ...
    <div ref={containerRef} style={{position:"fixed",inset:0,zIndex:400,background:"#000",display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden",touchAction:"none",userSelect:"none",WebkitUserSelect:"none",WebkitTouchCallout:"none"}}>
      ...
      {showChrome && (selEl || !hasPhoto || !layout.photo.show) && (
        <>
          ...
          <div style={{position:"absolute",right:8,top:"50%",transform:"translateY(-50%)",height:"60%",width:46,display:"flex",flexDirection:"column",alignItems:"center",zIndex:4}}>
    ...
    <div onClick={onClose} style={{position:"fixed",inset:0,zIndex:290}}>
      <div onClick={e=>e.stopPropagation()} style={{position:"fixed",top:"calc(env(safe-area-inset-top, 0px) + 62px)",right:"max(12px, calc(50vw - 320px))",zIndex:300,background:"rgba(var(--surface-rgb),0.82)",backdropFilter:"blur(6px) saturate(1.02)",WebkitBackdropFilter:"blur(6px) saturate(1.02)",border:"1px solid var(--border2)",borderRadius:8,padding:"14px 16px",width:236,boxShadow:"0 10px 34px rgba(0,0,0,0.6)"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr auto 1fr",alignItems:"center",marginTop:8}}>
          ...
          <button onClick={()=>{ setB(1.30); applyFilter(1.30); onCommit(1.30,t); }} style={pill}>Reset</button>
...
export default function App() {
  ...
  const [palette, setPalette] = useState(() => { const s = localStorage.getItem("selah_palette"); return s && PALETTES[s] ? s : "midnight"; });
  ...
  async function endSession() {
    ...
    const kidAge = ageFromBirthday(birthday);
    const kidNote = isKid ? ` This reader is a child${kidAge ? `, ${kidAge} years old` : " between 5 and 12 years old"}. In a kid profile the child's age is the single most important thing you calibrate to. It governs your vocabulary, your sentence length, how abstract you are allowed to be, and the overall temperature of everything you write. Write to ${kidAge ? `a ${kidAge}-year-old` : "this child's exact age"} specifically, not to "a kid" in general. A young child (about 5 to 7) needs very short sentences, the plainest words, concrete who-did-what questions, and any hard word explained the instant you use it. An older child (about 8 to 12) can hold longer sentences, real vocabulary, cause and effect, and questions that ask them to connect or compare what they read, still grounded in the text. Honor the reader: a well-read older child can be handed genuinely rich context and harder questions, so do not talk down to them or give a baby question like counting on fingers to a nine-year-old who knows his Bible. But never reach past the age either; keep the words, the examples, and the framing inside what a child that exact age actually understands. Match the age precisely. Explain hard words the moment they appear. Stay warm and honest, never water down the truth, just say it in words this child's age can hold. Never frighten or shame the child.` : "";
  ...
  return (
    <div className="app-root" style={{background:"var(--bg)",color:"var(--text)",fontFamily:"'Crimson Text',Georgia,serif",position:"relative"}}>
      <style>{`
        ...
        .nav-tab{background:transparent;border:none;color:var(--m1b);font-family:'Cinzel',serif;font-size:10px;font-weight:600;letter-spacing:0.12em;text-transform:uppercase;cursor:pointer;padding:10px 10px;border-bottom:2px solid transparent;transition:all 0.2s;flex:1;}
      ...
      <div className="app-container" style={{position:"relative",zIndex:1,flex:1,width:"100%",maxWidth:480,margin:"0 auto",padding:"0 16px calc(76px + env(safe-area-inset-bottom))",overflowAnchor:"none"}}>
        ...
        {view === "settings" && (
          <div className="fade-in">
            ...
            {/* Account */}
            <div className="card">
              <p className="label">Account<HelpDot show={!guidanceOff} text="Sign in to sync your reading log and settings across your devices. Without an account, everything stays only on this device." /></p>
              {account ? (
                <>
            ...
            {/* Bible Version */}
            <div className="card">
              <p className="label">Bible Translation<HelpDot show={!guidanceOff} text="Pick the translation you read in; the model follows its wording. Change it anytime, His Word does not change." /></p>
              <p style={{fontSize:15,color:"var(--m3)",lineHeight:1.6,marginBottom:14}}>
                {isKidAge
            ...
            {/* Gender + Age + Time */}
            <div className="card">
              <p className="label">I Am<HelpDot show={!guidanceOff} text="Your gender, age, and birthday let the model meet you where you are. It tunes the language and questions, never the truth." /></p>
              <p style={{fontSize:15,color:"var(--m3)",lineHeight:1.6,marginBottom:14}}>
            ...
            {/* Clock and Timezone */}
            <div className="card">
              <p className="label">Time Display<HelpDot show={!guidanceOff} text="Sets how session times show in your log: 12 or 24-hour, and your time zone if your device drifts near a border." /></p>
        ...
        <div ref={faithScrollRef} style={{position:"fixed",inset:0,zIndex:420,background:"var(--bg)",overflowY:"auto",WebkitOverflowScrolling:"touch",overscrollBehavior:"contain"}}>
          ...
          <div style={{maxWidth:640,margin:"0 auto",padding:"calc(env(safe-area-inset-top, 0px) + 78px) 24px calc(env(safe-area-inset-bottom, 0px) + 40px)"}}>
            ...
            <p style={{fontSize:11,lineHeight:1.65,color:"var(--m5)",marginTop:34,borderTop:"1px solid var(--border)",paddingTop:16}}>
              Scripture quotations are from the ESV® Bible (The Holy Bible, English Standard Version®), copyright © 2001 by Crossway, a publishing ministry of Good News Publishers. Used by permission. All rights reserved. Learn more at <a href="https://www.esv.org" target="_blank" rel="noopener" style={{color:"var(--accent)"}}>esv.org</a>.
            </p>
